...
func (c *Stdio) handleIncomingRequest(request JSONRPCRequest) {
	...
	go func() {
		...
		select {
		case <-ctx.Done():
			errorResponse := *NewJSONRPCErrorResponse(request.ID, INTERNAL_ERROR, ctx.Err().Error(), nil)
			c.sendResponse(errorResponse)
			return
		default:
		}
		...
		if err != nil {
			errorResponse := *NewJSONRPCErrorResponse(request.ID, INTERNAL_ERROR, err.Error(), nil)
			c.sendResponse(errorResponse)
			return
		}
