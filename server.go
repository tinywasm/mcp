...
func init() {
	var err error
	idGenerator, err = unixid.NewUnixID()
	if err != nil {
		panic(err)
	}
}
...
type ResourceHandlerFunc func(ctx context.Context, request ReadResourceRequest) ([]ResourceContents, error)
...
type PromptHandlerFunc func(ctx context.Context, request GetPromptRequest) (*GetPromptResult, error)
...
type TaskToolHandlerFunc func(ctx context.Context, request CallToolRequest) (*CreateTaskResult, error)
...
func WithResourceRecovery() ServerOption {
	return WithResourceHandlerMiddleware(func(next ResourceHandlerFunc) ResourceHandlerFunc {
		return func(ctx context.Context, request ReadResourceRequest) (result []ResourceContents, err error) {
			defer func() {
				if r := recover(); r != nil {
					err = fmt.Errorf(
...
func WithLogging() ServerOption {
	return func(s *MCPServer) {
		s.capabilities.logging = ToBoolPtr(true)
	}
}
...
func WithElicitation() ServerOption {
	return func(s *MCPServer) {
		s.capabilities.elicitation = ToBoolPtr(true)
	}
}
...
func WithRoots() ServerOption {
	return func(s *MCPServer) {
		s.capabilities.roots = ToBoolPtr(true)
	}
}
...
func WithCompletions() ServerOption {
	return func(s *MCPServer) {
		s.capabilities.completions = ToBoolPtr(true)
	}
}
...
func (s *MCPServer) AddResources(resources ...ServerResource) {
	...
	if s.capabilities.resources.listChanged {
		...
		s.SendNotificationToAllClients(MethodNotificationResourcesListChanged, nil)
	}
}
...
func (s *MCPServer) AddPrompts(prompts ...ServerPrompt) {
	...
	if s.capabilities.prompts.listChanged {
		// Send notification to all initialized sessions
		s.SendNotificationToAllClients(MethodNotificationPromptsListChanged, nil)
	}
}
...
func (s *MCPServer) SetPrompts(prompts ...ServerPrompt) {
	s.promptsMu.Lock()
	s.prompts = make(map[string]Prompt, len(prompts))
...
func (s *MCPServer) AddTaskTool(tool Tool, handler TaskToolHandlerFunc) {
...
func (s *MCPServer) handleInitialize(
	...
	if s.capabilities.tasks != nil {
		...
		if s.capabilities.tasks.toolCallTasks {
			tasksCapability.Requests = &TaskRequestsCapability{
				Tools: &struct {
...
func (s *MCPServer) handleSetLevel(
	...
	switch level {
	case LoggingLevelDebug, LoggingLevelInfo, LoggingLevelNotice,
		LoggingLevelWarning, LoggingLevelError, LoggingLevelCritical,
		LoggingLevelAlert, LoggingLevelEmergency:
		// Valid level
	default:
		return nil, &requestError{
			id:   id,
...
func (s *MCPServer) handleListResources(
...
) (*ListResourcesResult, *requestError) {
	s.resourcesMu.RLock()
	resourceMap := make(map[string]Resource, len(s.resources))
	...
	resourcesList := slices.SortedFunc(maps.Values(resourceMap), func(a, b Resource) int {
...
func (s *MCPServer) handleListResourceTemplates(
...
) (*ListResourceTemplatesResult, *requestError) {
	// Get global templates
	s.resourcesMu.RLock()
	templateMap := make(map[string]ResourceTemplate, len(s.resourceTemplates))
	...
	templates := make([]ResourceTemplate, 0, len(templateMap))
...
) (*ReadResourceResult, *requestError) {
	s.resourcesMu.RLock()
...
func matchesTemplate(uri string, template *URITemplate) bool {
...
) (*ListPromptsResult, *requestError) {
	s.promptsMu.RLock()
...
func (s *MCPServer) handleListTools(
	...
	s.toolsMu.RLock()
	tools := make([]Tool, 0, len(s.tools)+len(s.taskTools))
...
func (s *MCPServer) handleToolCall(
	...
	if tool.Tool.Execution != nil && tool.Tool.Execution.TaskSupport == TaskSupportRequired {
	...
	shouldExecuteAsTask := request.Params.Task != nil &&
		...
		(tool.Tool.Execution.TaskSupport == TaskSupportOptional ||
			tool.Tool.Execution.TaskSupport == TaskSupportRequired)
...
func (s *MCPServer) handleTaskAugmentedToolCall(
	...
	if isTaskTool {
		...
		if regularTool.Tool.Execution == nil ||
			(regularTool.Tool.Execution.TaskSupport != TaskSupportOptional &&
				regularTool.Tool.Execution.TaskSupport != TaskSupportRequired) {
			return nil, &requestError{
				id:   id,
				code: METHOD_NOT_FOUND,
...
func (s *MCPServer) executeTaskTool(
	...
	if err != nil {
		...
		if errors.Is(err, context.Canceled) || errors.Is(err, context.DeadlineExceeded) {
			...
			s.tasksMu.Lock()
			alreadyCancelled := entry.task.Status == TaskStatusCancelled
			s.tasksMu.Unlock()
...
func (s *MCPServer) handleTaskResult(
	...
	if callToolResult, ok := storedResult.(*CallToolResult); ok {
...
func (s *MCPServer) handleCancelTask(
	...
	result := NewCancelTaskResult(task)
	return &result, nil
}
...
func (s *MCPServer) createTask(ctx context.Context, taskID string, toolName string, ttl *int64, pollInterval *int64) (*taskEntry, error) {
	...
	opts := []TaskOption{}
	if ttl != nil {
		opts = append(opts, WithTaskTTL(*ttl))
	}
	if pollInterval != nil {
		opts = append(opts, WithTaskPollInterval(*pollInterval))
	}
	task := NewTask(taskID, opts...)
	createdAt := time.Now()
...
func (s *MCPServer) getTask(ctx context.Context, taskID string) (Task, chan struct{}, error) {
	s.tasksMu.RLock()
	...
	if !exists {
		...
		if _, wasExpired := s.expiredTasks[taskID]; wasExpired {
			s.tasksMu.RUnlock()
			return Task{}, nil, fmt.Errorf("task has expired")
		}
		s.tasksMu.RUnlock()
		return Task{}, nil, fmt.Errorf("task not found")
	}
...
func (s *MCPServer) listTasks(ctx context.Context) []Task {
	sessionID := getSessionID(ctx)
...
func (s *MCPServer) completeTask(entry *taskEntry, result any, err error) {
	...
	if err != nil {
		entry.task.Status = TaskStatusFailed
		entry.task.StatusMessage = err.Error()
		entry.resultErr = err
	} else {
		entry.task.Status = TaskStatusCompleted
		entry.result = result
	}
...
func (s *MCPServer) sendTaskStatusNotification(task Task) {
	...
	s.SendNotificationToAllClients(MethodNotificationTasksStatus, taskMap)
}
