...
import (
	...
	"github.com/tinywasm/mcp/internal/uritemplate"
)
...
const (
	...
	// MethodPing verifies connection liveness between client and
...
type ClientCapabilities struct {
	...
	// Present if the client supports elicitation requests from the
...
type ServerCapabilities struct {
	...
	Experimental map[string]any `json:"experimental,omitempty"`
	// Present if the server supports sending log messages to the
	...
	// Present if the server supports elicitation requests to the
	Elicitation *ElicitationCapability `json:"elicitation,omitempty"`
	// Present if the server supports roots requests to the
	Roots *struct{} `json:"roots,omitempty"`
	// Present if the server supports task-based execution.
	Tasks *TasksCapability `json:"tasks,omitempty"`
	// Present if the server supports completions requests to the
...
// resources/templates/list request from the
...
// resources/updated notifications from the  This should follow a previous
...
// server to  If no logging/setLevel request has been sent from the client,
...
// request from the  The client should inform the user before returning the
...
//
//	The client can use annotations to inform how objects are used or
//
// displayed
...
// ListRootsRequest is sent from the server to request a list of root URIs from the  Roots allow
...
// ClientRequest represents any request that can be sent from client to
type ClientRequest any

// ClientNotification represents any notification that can be sent from client to
type ClientNotification any

// ClientResult represents any result that can be sent from client to
type ClientResult any

// ServerRequest represents any request that can be sent from server to
type ServerRequest any

// ServerNotification represents any notification that can be sent from server to
type ServerNotification any

// ServerResult represents any result that can be sent from server to
...
// ElicitationCapability represents the elicitation capabilities of a client or
